...
class CodigoPostalScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PlantillaVentanas(
      ...
      columns: const [
        DataColumn(label: Text('CÓDIGO POSTAL', style: TextStyle(fontWeight: FontWeight.bold))),
        DataColumn(label: Text('LOCALIDAD ASIGNADA', style: TextStyle(fontWeight: FontWeight.bold))),
      ...
      rows: cpAsync.maybeWhen(
        data: (codigos) => codigos.map((cp) {
          ...
          return DataRow(cells: [
            ...
            DataCell(Row(
              children: [
                IconButton(icon: const Icon(Icons.edit_note, color: Colors.blue), onPressed: () => _showSideForm(context, ref, cp: cp)),
                IconButton(icon: const Icon(Icons.delete_sweep_outlined, color: Colors.red), onPressed: () => _confirmDelete(context, ref, cp)),
              ],
            )),
          ]);
        }).toList(),
  ...
  void _showSideForm(BuildContext context, WidgetRef ref, {CodigoPostal? cp}) {
    showGeneralDialog(
      ...
      transitionBuilder: (context, anim1, anim2, child) => SlideTransition(position: Tween<Offset>(begin: const Offset(1, 0), end: Offset.zero).animate(anim1), child: child),
    );
  }
...
class _CPFormContentState extends ConsumerState<_CPFormContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(32), child: Form(key: formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _label("CÓDIGO POSTAL"),
        TextFormField(controller: cpCtrl, keyboardType: TextInputType.number, decoration: _inputDecoration(Icons.mark_as_unread_outlined), validator: (v) => v!.isEmpty ? 'Requerido' : null),
        const SizedBox(height: 25),
        _label("LOCALIDAD"),
        ref.watch(localidadesProvider).when(
          data: (list) => DropdownButtonFormField<int>(value: selectedLoc, decoration: _inputDecoration(Icons.location_city), items: list.map((l) => DropdownMenuItem(value: l.id, child: Text(l.nombreLocalidad))).toList(), onChanged: (val) => setState(() => selectedLoc = val)),
          loading: () => const LinearProgressIndicator(),
          error: (_, __) => const Text("Error"),
        ),
        const SizedBox(height: 50),
        SizedBox(width: double.infinity, height: 50, child: FilledButton(style: FilledButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), onPressed: _save, child: Text(widget.cp == null ? "GUARDAR" : "ACTUALIZAR"))),
      ]))))
    ]);
  }
  ...
  void _save() async {
    ...
    if (widget.cp == null) {
      await ref.read(codigosPostalesProvider.notifier).agregarCodigoPostal(cpCtrl.text, selectedLoc!);
    } else {
      await ref.read(codigosPostalesProvider.notifier).editarCodigoPostal(widget.cp!.id, cpCtrl.text, selectedLoc!);
    }
    if (mounted) Navigator.pop(context);
  }

  Widget _buildHeader(BuildContext context, ColorScheme colors, bool isNew) => Container(padding: const EdgeInsets.fromLTRB(24, 40, 16, 20), color: colors.primary.withOpacity(0.08), child: Row(children: [Icon(isNew ? Icons.post_add : Icons.edit, color: colors.primary), const SizedBox(width: 12), Text(isNew ? 'Nuevo C.P.' : 'Editar C.P.', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: colors.primary)), const Spacer(), IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close))]));
