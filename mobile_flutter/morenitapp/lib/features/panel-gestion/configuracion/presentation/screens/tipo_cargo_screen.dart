    ...
    return PlantillaVentanas(
      ...
      rows: cargosAsync.maybeWhen(
        data: (lista) => lista.map((c) => DataRow(cells: [
          ...
          DataCell(Text(c.observaciones ?? '-', overflow: TextOverflow.ellipsis)),
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(isNew ? Icons.work_outline : Icons.edit_note, color: colors.primary, size: 28),
          ...
          child: SingleChildScrollView(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  _buildField("CÓDIGO DEL CARGO", codCtrl, "Ej: CAR-01", colors),
                  const SizedBox(height: 25),
                  _buildField("NOMBRE DEL CARGO", nomCtrl, "Director, Gerente, etc.", colors),
                  const SizedBox(height: 25),
                  _buildField("OBSERVACIONES", obsCtrl, "Notas adicionales sobre este cargo", colors, maxLines: 4),
  ...
  Widget _buildField(String label, TextEditingController ctrl, String hint, ColorScheme colors, {int maxLines = 1}) {
    return Column(
      ...
      children: [
        Text(
          label, 
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: colors.primary, letterSpacing: 1.1)
        ),
        ...
        TextFormField(
          ...
          validator: (v) => v!.trim().isEmpty ? 'Este campo es requerido' : null
        ),
      ]
    );
  }
      ...
      child: FilledButton(
        ...
        onPressed: () {
          ...
          if (widget.cargo == null) {
            notifier.crear(codCtrl.text.trim(), nomCtrl.text.trim(), obsCtrl.text.trim());
          } else {
            notifier.editar(widget.cargo.id, codCtrl.text.trim(), nomCtrl.text.trim(), obsCtrl.text.trim());
          }
          
          Navigator.pop(context);
          
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(widget.cargo == null ? 'Cargo creado' : 'Cargo actualizado'))
          );
        }, 
        child: Text(
