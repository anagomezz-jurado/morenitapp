...
class UserMapper {
  ...
  String odooString(dynamic val) {
    ...
    if (val is List && val.length > 1) return val[1].toString(); // Para campos Many2one
    return val.toString();
  }
  ...
  debugPrint('>>> numero_hermano raw desde Odoo: $raw'); // Quitar tras depurar
  if (raw == null || raw == false || raw.toString() == 'false' || raw.toString().isEmpty) {
    return 'No vinculado';
  }
  return raw.toString();
}(),
